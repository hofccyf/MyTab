 ...
 * 6. 音乐模块支持两种引擎（设置面板可切换）：
 *    - 洛雪音乐（LX Music）：在设置里填入自建/公共 lx-music-api-server 地址，更稳定，推荐。如果没有lx-music-api-server，就部署下方的音源适配.js，在导航页设置里填写音源适配.js的workers.dev域名即可！
 *      LX API 协议（兼容 lx-music-api-server 项目）：
 *        GET {lxUrl}/search?source={kw|tx|wy|mg}&keyword=&count=
 *        GET {lxUrl}/url?source={kw|tx|wy|mg}&id=&quality=320k
 *        GET {lxUrl}/pic?source={kw|tx|wy|mg}&id=
 *      返回结构统一为 { code:0, data:[...] / data:{ url, ... } }
 *    - 内置 API（gdstudio）：保留作为可选，未配置 LX 时使用。
...
export default {
  async fetch(request, env, ctx) {
    ...
    // === API: 音乐跨域 - 搜索列表 ===
    // 引擎选择：engine=lx 走洛雪音乐（直接调用 lx-music-api-server，失败不降级）
    //           engine=builtin 走内置 gdstudio + 网易云 fallback
    if (url.pathname === '/api/music') {
        try {
            const source  = url.searchParams.get('source')  || 'kuwo'; // 内置引擎默认源: netease、kuwo、joox、bilibili
            const keyword = url.searchParams.get('keyword') || '洋澜一';
            const count   = url.searchParams.get('count')   || '18';
            const engine  = url.searchParams.get('engine')  || 'builtin';
            const lxUrl   = url.searchParams.get('lxUrl')   || '';
            const lxSource = url.searchParams.get('source') || url.searchParams.get('lxSource') || 'kw';

            // ---------- 洛雪音乐模式：只调 LX 服务器，不降级 ----------
            if (engine === 'lx' && lxUrl) {
                try {
                    const baseUrl = lxUrl.replace(/\/+$/, '');
                    const apiUrl = baseUrl + '/search?source=' + encodeURIComponent(lxSource) +
                                   '&keyword=' + encodeURIComponent(keyword) + '&count=' + encodeURIComponent(count);
                    console.log('[music] LX search calling:', apiUrl);
                    const lxRes = await Promise.race([
                        fetch(apiUrl, {
                            headers: { 'Accept': 'application/json' },
                            redirect: 'follow',
                            cf: { cacheEverything: false, cacheTtl: 0 }
                        }),
                        new Promise((_, r) => setTimeout(() => r(new Error('lx timeout')), 10000))
                    ]);
                    if (!lxRes.ok) throw new Error('lx search HTTP ' + lxRes.status);
                    const lxData = await lxRes.json();
                    console.log('[music] LX search response code:', lxData && lxData.code, 'dataLen:', Array.isArray(lxData && lxData.data) ? lxData.data.length : 'N/A');
                    const ok = lxData && (lxData.code === 0 || lxData.code === 200);
                    const list = ok && Array.isArray(lxData.data) ? lxData.data
                               : (ok && lxData.data && Array.isArray(lxData.data.list) ? lxData.data.list
                               : (lxData && Array.isArray(lxData.list) ? lxData.list : null));
                    if (!list || list.length === 0) throw new Error('lx empty');

                    const tracks = list.map(item => {
                        const s = item.singer;
                        let artist = '未知';
                        if (typeof s === 'string') artist = s;
                        else if (Array.isArray(s)) artist = s.map(x => typeof x === 'string' ? x : (x && x.name || '')).filter(Boolean).join(' / ') || '未知';
                        return {
                            id:      String(item.songId || item.id || ''),
                            name:    item.songName || item.name || '未知',
                            artist:  artist,
                            pic_id:  String(item.songId || item.id || ''),
                            source:  lxSource,
                            pic_url: item.picUrl || item.albumImg || '',
                            album:   item.albumName || item.album || ''
                        };
                    }).filter(t => t.id && t.name);

                    if (tracks.length === 0) throw new Error('lx no valid tracks');
                    return new Response(JSON.stringify(tracks), {
                        headers: { 'content-type': 'application/json;charset=UTF-8', 'Access-Control-Allow-Origin': '*' }
                    });
                } catch (lxErr) {
                    console.warn('[music] LX search failed:', lxErr.message);
            ...
            // ---------- 内置 API 模式：gdstudio → 网易云 fallback ----------
            let tracks = [];
            try {
    ...
    if (url.pathname === '/api/music/url') {
        try {
            ...
            const engine = url.searchParams.get('engine') || 'builtin';
            const lxUrl  = url.searchParams.get('lxUrl') || '';
            // 优先用 source 参数（前端传的 t.source 可能更准确），其次 lxSource
            const lxSource = url.searchParams.get('source') || url.searchParams.get('lxSource') || 'kw';
            const quality = url.searchParams.get('quality') || '320k';
            if (!id) {
                console.warn('[music] /api/music/url: missing id param', {
            ...
            if (engine === 'lx' && lxUrl) {
                try {
                    const baseUrl = lxUrl.replace(/\/+$/, '');
                    const apiUrl = baseUrl + '/url?source=' + encodeURIComponent(lxSource) +
                                   '&id=' + encodeURIComponent(id) + '&quality=' + encodeURIComponent(quality);
                    console.log('[music] LX url calling:', apiUrl);
                    const lxRes = await Promise.race([
                        fetch(apiUrl, {
                            headers: { 'Accept': 'application/json' },
                            redirect: 'follow',
                    ...
                    if (!lxRes.ok) throw new Error('lx url HTTP ' + lxRes.status);
                    const lxData = await lxRes.json();
                    console.log('[music] LX url response:', JSON.stringify(lxData).slice(0, 200));
                    const ok = lxData && (lxData.code === 0 || lxData.code === 200);
                    const u = ok && lxData.data ? (lxData.data.url || (Array.isArray(lxData.data) && lxData.data[0] ? lxData.data[0].url : '')) : '';
                    if (u) {
                        return new Response(JSON.stringify({ url: u }), {
                            headers: { 'content-type': 'application/json;charset=UTF-8', 'Access-Control-Allow-Origin': '*' }
                        });
                    }
                    throw new Error('lx no url, resp=' + JSON.stringify(lxData).slice(0, 200));
                } catch (lxErr) {
                    console.warn('[music] LX url failed:', lxErr.message);
                    return new Response(JSON.stringify({ url: '' }), {
                        headers: { 'content-type': 'application/json', 'Access-Control-Allow-Origin': '*' }
                    });
                }
            ...
            // ---------- 内置 API 模式：gdstudio → injahow → i-meto → 网易云直链 ----------
            let finalUrl = '';
            try {
                const res = await Promise.race([
    ...
    // === API: 音乐 - 获取封面图 (洛雪音乐专用直连) ===
    if (url.pathname === '/api/music/pic') {
        try {
            const lxUrl = url.searchParams.get('lxUrl') || '';
            const lxSource = url.searchParams.get('lxSource') || 'kw';
            const id = url.searchParams.get('id');
            if (!lxUrl || !id) throw new Error('no params');
            const apiUrl = lxUrl.replace(/\/+$/, '') + '/pic?source=' + encodeURIComponent(lxSource) + '&id=' + encodeURIComponent(id);
            const res = await fetch(apiUrl, { headers: { 'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json' } });
            const data = await res.json();
            const u = data && (data.code === 0 || data.code === 200) && data.data ? (data.data.url || data.data) : '';
            return new Response(JSON.stringify({ url: u }), {
                headers: { 'content-type': 'application/json;charset=UTF-8', 'Access-Control-Allow-Origin': '*' }
            });
        } catch (e) {
    ...
    if (!userConfig) {
      ...
      userConfig = {
        ...
        "music": {
            "engine": "builtin",     // "lx" 走洛雪音乐，"builtin" 走内置 gdstudio
            "lxUrl": "",             // 洛雪音乐 API 地址，engine=lx 时必填
            "lxSource": "kw",        // 洛雪子源: kw(酷我) / tx(腾讯) / wy(网易云) / mg(咪咕)
            "builtinSource": "kuwo",  // 内置引擎子源: netease / kuwo / joox / bilibili
            "quality": "320k"       // 音质: 128k / 320k / flac
        },
    ...
    // 兼容老配置：自动补全 music 字段
    if (!userConfig.music || typeof userConfig.music !== 'object') userConfig.music = {};
    const _m = userConfig.music;
    if (!_m.engine)  _m.engine = 'builtin';
    if (!_m.lxUrl)   _m.lxUrl = '';
    if (!_m.lxSource) _m.lxSource = 'kw';
    if (!_m.builtinSource) _m.builtinSource = 'kuwo';
    if (!_m.quality) _m.quality = '320k';
...
<body>
    ...
    <div class="modal" id="configModal">
        <div class="modal-content">
            ...
            <div id="settings-form" style="display:none;">
                ...
                <div class="form-group" style="margin-top:18px; padding-top:12px; border-top:1px dashed #444;">
                    <label>🎵 音乐引擎</label>
                    <select id="music-engine" onchange="toggleMusicEngine()">
                        <option value="builtin">内置 API (gdstudio)</option>
                        <option value="lx">洛雪音乐 (自定义源)</option>
                    </select>
                </div>
                <div id="music-lx-group" style="display:none;">
                    <div class="form-group"><label>洛雪 API 地址</label><input type="text" id="music-lx-url" placeholder="https://api.example.com  (自建/公共 LX API 服务器)"></div>
                    <div class="form-group"><label>洛雪子源</label><select id="music-lx-source"><option value="kw">酷我 (kw)</option><option value="tx">腾讯 (tx)</option><option value="wy">网易云 (wy)</option><option value="mg">咪咕 (mg)</option></select></div>
                </div>
                <div id="music-builtin-group">
                    <div class="form-group"><label>内置 API 子源</label><select id="music-builtin-source"><option value="kuwo">酷我 (kuwo)</option><option value="netease">网易云 (netease)</option><option value="joox">JOOX (joox)</option><option value="bilibili">B站 (bilibili)</option></select></div>
                </div>
                <div class="form-group"><label>音质</label><select id="music-quality"><option value="128k">标准 128k</option><option value="320k">高品 320k</option><option value="flac">无损 FLAC</option></select></div>
                <div style="font-size:0.78rem; color:#888; padding:6px 4px 10px; line-height:1.5;">
                    💡 提示：洛雪音乐引擎需自建或使用公共 LX API 服务器（如 <span style="color:#4aa;">lx-music-api-server</span> 项目）。LX 失败时会自动降级到内置 API。
                </div>
    ...
    <script>
        ...
        /* === 👇 可调参数 5：音乐播放器设置 👇 ========================== */
        /* ===================================================================== */
        // 注意：音乐源现在由"设置面板 → 音乐引擎"动态控制，会覆盖此处默认值。
        // 这里的常量仅作为：
        //   - 首次访问（KV 还没有 config.music 时的兜底默认源）
        //   - 搜索框为空时的初始关键词 / 加载数量
        const MUSIC_SOURCE  = "kuwo";  // 内置引擎兜底源: netease、kuwo、joox、bilibili
...
">在新标签页中翻译</button></div>\`;
        ...
        // ==================== 跨域直连 APlayer 引擎 ====================
        // 优先使用 config.music 中的引擎配置；如果没配置则自动 fallback 到内置 API。
        function getMusicConfig() {
            const m = (config.music && typeof config.music === 'object') ? config.music : {};
            return {
                engine:        m.engine || 'builtin',
                lxUrl:         m.lxUrl || '',
                lxSource:      m.lxSource || 'kw',
                builtinSource: m.builtinSource || 'kuwo',
                quality:       m.quality || '320k'
            };
        }
        function buildMusicQuery(extra) {
            const m = getMusicConfig();
            const params = new URLSearchParams();
            if (m.engine === 'lx' && m.lxUrl) {
                params.set('engine', 'lx');
                params.set('lxUrl', m.lxUrl);
                params.set('lxSource', m.lxSource);
                params.set('quality', m.quality);
            } else {
                params.set('engine', 'builtin');
                params.set('source', m.builtinSource);
            }
        ...
        async function initMusicPlayer(id, keyword) {
            ...
            try {
                const engineLabel = (m.engine === 'lx' && m.lxUrl) ? ('洛雪·' + m.lxSource.toUpperCase()) : ('内置·' + m.builtinSource);
                container.innerHTML = '<div style="padding: 20px; text-align: center; color: #aaa;">🔍 [' + engineLabel + '] 搜索中：' + kw + '...</div>';

                // Step1: 获取歌曲列表
                const listRes = await fetch('/api/music?' + buildMusicQuery({ keyword: kw, count: MUSIC_COUNT }));
                const tracks = await listRes.json();
                if(!Array.isArray(tracks) || tracks.length === 0) throw new Error('empty');

                // Step2: 立即用「未加载」占位创建 APlayer，让用户先看到歌单
                container.innerHTML = '';
                ...
                // Step3: 并发获取每首歌的播放URL（限并发 5 个，避免触发上游限流）
                // 命中 KV 缓存的会秒回，未命中的才真正调用上游
                const CONCURRENCY = 5;
                let completed = 0;
                let okCount = 0;
                const total = tracks.length;
                const statusEl = document.createElement('div');
                statusEl.style.cssText = 'position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.6);color:#ffeb3b;font-size:0.75rem;padding:2px 8px;border-radius:10px;z-index:10;';
                container.style.position = 'relative';
                container.appendChild(statusEl);
                statusEl.textContent = '⏳ 加载中 0/' + total;

                async function processOne(t, idx) {
                    try {
                        const urlQuery = buildMusicQuery({ id: t.id, source: t.source || (m.engine === 'lx' ? m.lxSource : m.builtinSource) });
                        const r = await fetch('/api/music/url?' + urlQuery);
                        const d = await r.json();
                        if (!d.url) { completed++; updateStatus(); return null; }

                        // 封面：优先用搜索返回的 pic_url；否则洛雪走 /api/music/pic，内置走 gdstudio pic
                        let cover = t.pic_url || '';
                        if (!cover && t.pic_id) {
                            if (m.engine === 'lx' && m.lxUrl) {
                                try {
                                    const pcRes = await fetch('/api/music/pic?lxUrl=' + encodeURIComponent(m.lxUrl) + '&lxSource=' + encodeURIComponent(t.source || m.lxSource) + '&id=' + encodeURIComponent(t.pic_id));
                                    const pcData = await pcRes.json();
                                    if (pcData && pcData.url) cover = pcData.url;
                                } catch(e) {}
                            }
                            if (!cover) cover = PIC_BASE + '&source=' + encodeURIComponent(t.source || m.builtinSource) + '&id=' + t.pic_id;
                        }

                        // 动态更新 APlayer 这首歌的 url 和 cover
                        try {
                            ap.list.audios[idx].url = d.url;
                            if (cover) ap.list.audios[idx].cover = cover;
                        } catch(e) {}
                        completed++;
                        okCount++;
                        updateStatus();
                ...
                function updateStatus() {
                    statusEl.textContent = '⏳ 加载中 ' + completed + '/' + total + ' (可用 ' + okCount + ')';
                    if (completed >= total) {
                        if (okCount === 0) {
                            statusEl.textContent = '❌ 全部失败';
                            statusEl.style.color = '#ff4757';
                        } else {
                            statusEl.textContent = '✅ 已加载 ' + okCount + '/' + total;
                            statusEl.style.color = '#52c41a';
                            // 用户当前正在播放这首歌时，立刻设置 url 让它继续
                        }
                    }
                }

                // 并发池：限制最多同时 CONCURRENCY 个请求
                const queue = tracks.map((t, idx) => ({ t, idx }));
                const workers = [];
                for (let i = 0; i < Math.min(CONCURRENCY, queue.length); i++) {
                    workers.push((async () => {
                        while (queue.length > 0) {
                            ...
                            if (!item) break;
                            await processOne(item.t, item.idx);
                        }
                    })());
                }
                ...
                // 如果所有歌曲都失败，提示用户
                if (okCount === 0) {
                    container.innerHTML = '<div style="padding: 20px; text-align: center; color: #ff4757;">所有歌曲加载失败，请检查网络或更换子源</div>';
                }
            } catch(e) {
        ...
        function openSettingsModal() { 
            ...
            document.getElementById('music-engine').value = m.engine || 'builtin';
            document.getElementById('music-lx-url').value = m.lxUrl || '';
            document.getElementById('music-lx-source').value = m.lxSource || 'kw';
            document.getElementById('music-builtin-source').value = m.builtinSource || 'kuwo';
            document.getElementById('music-quality').value = m.quality || '320k';
            toggleMusicEngine();
        }
        function toggleMusicEngine() {
            const engine = document.getElementById('music-engine').value;
            document.getElementById('music-lx-group').style.display = (engine === 'lx') ? 'block' : 'none';
            document.getElementById('music-builtin-group').style.display = (engine === 'builtin') ? 'block' : 'none';
        }
        ...
        async function saveSettingsData() {
            ...
            // 保存音乐引擎配置
            const engine = document.getElementById('music-engine').value;
            const lxUrl = document.getElementById('music-lx-url').value.trim();
            if (engine === 'lx' && !lxUrl) {
                if(!confirm('已选择洛雪音乐引擎但未填 API 地址，将自动降级到内置 API。是否继续？')) return;
            }
            if (!config.music || typeof config.music !== 'object') config.music = {};
            config.music.engine = engine;
            config.music.lxUrl = lxUrl;
            config.music.lxSource = document.getElementById('music-lx-source').value;
            config.music.builtinSource = document.getElementById('music-builtin-source').value;
            config.music.quality = document.getElementById('music-quality').value;
